/*
 * File: IAS-LangLib/src/lang/export/text/expr/RelationalIsTypeNodeHandler.cpp
 * 
...
void RelationalIsTypeNodeHandler::call(const Model::Node* pNode, CallbackCtx *pCtx, CallbackSignature::Result& aResult){
	...
	handleUnary(pRelationalIsTypeNode->getChild(),pCtx,aResult,"");

	aResult.ssOutput<<" ISTYPE (";

	aResult.ssOutput<<pRelationalIsTypeNode->getTypeInfoNode()->getType();

	if(pRelationalIsTypeNode->getTypeInfoNode()->hasNamespace())
		aResult.ssOutput<<": \""<<pRelationalIsTypeNode->getTypeInfoNode()->getNamespace() <<"\"";
