/*
 * File: IAS-LangLib/src/lang/interpreter/proc/expr/ModuloNodeHandler.cpp
 * 
...
void ModuloNodeHandler::call(const Model::Node* pNode, CallbackCtx *pCtx, CallbackSignature::Result& aResult){
	...
	CallbackRegister::SubCall(pModuloNode->getLeftExpr(),pCtx,aResultLeft);
	IAS_DFT_FACTORY<Exe::Expr::Expr>::PtrHolder ptrExprLeft(aResultLeft.pExpr);

	CallbackSignature::Result aResultRight;
	CallbackRegister::SubCall(pModuloNode->getRightExpr(),pCtx,aResultRight);
