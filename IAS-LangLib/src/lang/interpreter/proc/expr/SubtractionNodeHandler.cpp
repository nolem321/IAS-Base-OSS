/*
 * File: IAS-LangLib/src/lang/interpreter/proc/expr/SubtractionNodeHandler.cpp
 * 
...
void SubtractionNodeHandler::call(const Model::Node* pNode, CallbackCtx *pCtx, CallbackSignature::Result& aResult){
	...
	CallbackRegister::SubCall(pSubtractionNode->getLeftExpr(),pCtx,aResultLeft);
	IAS_DFT_FACTORY<Exe::Expr::Expr>::PtrHolder ptrExprLeft(aResultLeft.pExpr);

	CallbackSignature::Result aResultRight;
	CallbackRegister::SubCall(pSubtractionNode->getRightExpr(),pCtx,aResultRight);
