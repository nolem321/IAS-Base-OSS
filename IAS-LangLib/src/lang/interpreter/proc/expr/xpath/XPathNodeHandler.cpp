/*
 * File: IAS-LangLib/src/lang/interpreter/proc/expr/xpath/XPathNodeHandler.cpp
 * 
...
void XPathNodeHandler::call(const Model::Node* pNode, CallbackCtx *pCtx, CallbackSignature::Result& aResult){
	...
	IAS_DFT_FACTORY<Exe::Expr::XPath::XPathAccessor>::PtrHolder ptrAccessor(IAS_DFT_FACTORY<Exe::Expr::XPath::XPathAccessor>::Create());

	for(Model::Expr::XPath::XPathNode::ElementList::const_iterator it=lstElements.begin();
		it != lstElements.end();
		it++){

		IAS_LOG(::IAS::Lang::LogLevel::INSTANCE.isDetailedInfo(),"(*it)="<<(*it)->getName());

		if((*it)->isMulti()){
			CallbackSignature::Result aTmpResult;
   ...
   aResult.pXPathAccessor=ptrAccessor.pass();
   IAS_LOG(::IAS::Lang::LogLevel::INSTANCE.isDetailedInfo(),"done!, "<<lstElements.size());
}
/*************************************************************************/
}
}
