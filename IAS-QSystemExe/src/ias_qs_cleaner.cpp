/*
 * File: IAS-QSystemExe/src/ias_qs_cleaner.cpp
 * 
...
#include "parameters/CleanerParameters.h"
#include <qs/api.h>
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		IAS_DFT_FACTORY<CleanerParameters>::PtrHolder
			ptrParameters(IAS_DFT_FACTORY<CleanerParameters>::Create(argc, argv));

		if (ptrParameters->checkAndShowHelp(std::cerr))
			return 1;

		SHM::Administrator::StartCleaner(ptrParameters->getSystemName());
