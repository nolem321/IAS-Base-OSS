/*
 * File: IAS-QSystemExe/src/ias_qs_create.cpp
 * 
...
#include "parameters/CreateParameters.h"
#include "qs/api.h"
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		IAS_DFT_FACTORY<CreateParameters>::PtrHolder ptrParameters(IAS_DFT_FACTORY<CreateParameters>::Create(argc, argv));

		if (ptrParameters->checkAndShowHelp(std::cerr))
			return 1;

		SHM::Administrator::CreateSystem(ptrParameters->getSystemDescription());
