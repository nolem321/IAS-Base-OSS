/*
 * File: IAS-QSystemExe/src/ias_qs_status.cpp
 * 
...
#include "parameters/StatusParameters.h"
#include "qs/api.h"
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		IAS_DFT_FACTORY<StatusParameters>::PtrHolder ptrParameters(IAS_DFT_FACTORY<StatusParameters>::Create(argc, argv));

		if (ptrParameters->checkAndShowHelp(std::cerr))
			return 1;

		SHM::Administrator::DisplaySystem(ptrParameters->getSystemName(),std::cerr);
