...
void SOAPFormatter::read(DM::DataObjectPtr& dmData,
	...
	IAS_LOG(QS::LogLevel::INSTANCE.isData()||true,"SOAP data: " + ss.str());
	...
	IAS_DFT_FACTORY<Parser>::PtrHolder ptrParser(IAS_DFT_FACTORY<Parser>::Create(ptrXMLHelper,
			IAS_DFT_FACTORY<DM::XML::LibXMLLexerForStream>::Create<std::istream&>(ss),
			pAttributes));
...
/*************************************************************************/
void SOAPFormatter::getSOAPContent(std::istream& istream,
...
/*************************************************************************/
extern "C"{
void* _ias_qs_fmt_SOAP_12(const ::IAS::DM::DataFactory* pDataFactory){
	IAS_TRACER;
