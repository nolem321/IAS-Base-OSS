...
#include <org/invenireaude/qsystem/workers/ds/Autogenerate.h>
...
DSFactory::DSFactory(const ::org::invenireaude::qsystem::workers::ds::Specification* dmSpecification,
  ...
  ptrAutoGeneratedSpecification = ::org::invenireaude::qsystem::workers::ds::DataFactory::GetInstance()
                        ->getSpecificationType()->createSpecification();

	IAS_LOG(IAS::QS::LogLevel::INSTANCE.isInfo(),"DataSources: "<<lstDataSources.size());

	for(int iIdx=0;iIdx<lstDataSources.size();iIdx++){
    const ds::Parameter* pParameter(lstDataSources.at(iIdx));

    if(pParameter->isSetAutogenerate()){

      for(int iIdx = pParameter->getAutogenerate()->getFrom(); iIdx <= pParameter->getAutogenerate()->getTo(); iIdx++){
         ds::Ext::ParameterPtr ptrNewParameter = pParameter->duplicateParameter();
         ptrNewParameter->setName(pParameter->getName()+"."+TypeTools::IntToString(iIdx));
         ptrNewParameter->getConnection()->setLocation(TypeTools::Replace(
           pParameter->getConnection()->getLocation(),"%%",TypeTools::IntToString(iIdx)));


        if(hmDataSources.count(ptrNewParameter->getName()))
			    IAS_THROW(BadUsageException("Duplicated datasource: ")<<ptrNewParameter->getName());

        ptrAutoGeneratedSpecification->getDatasourcesList().add(ptrNewParameter);
        hmDataSources[ptrNewParameter->getName()]=ptrNewParameter;

        IAS_LOG(IAS::QS::LogLevel::INSTANCE.isInfo(),"Auto generated connection: "+ptrNewParameter->getName());
      }

    }else{

    if(hmDataSources.count(pParameter->getName()))
