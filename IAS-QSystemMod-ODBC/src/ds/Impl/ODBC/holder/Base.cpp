 ...
 *
 * File: IAS-QSystemMod-ODBC/src/ds/Impl/ODBC/holder/Base.cpp
 *
...
void Base::define(int iPos){
	...
	IAS_LOG(LogLevel::INSTANCE.isInfo(),"define: "<<iPos<<", len="<<bufData.getSize());

  pStatement->checkForErrors(
        pStatement->getSQLText()+", def="+TypeTools::IntToString(iPos),
        SQLBindCol((SQLHANDLE)*pStatement,
							   iPos,
							   iCType,
...
void Base::bind(int iPos,SQLSMALLINT iBindType){
	...
	IAS_LOG(LogLevel::INSTANCE.isInfo(),"bind: "<<iPos<<", len="<<bufData.getSize());

  pStatement->checkForErrors(
        pStatement->getSQLText()+", def="+TypeTools::IntToString(iPos),
        SQLBindParameter((SQLHANDLE)*pStatement,
									iPos,
									iBindType,
									iCType,
									iSQLType,
